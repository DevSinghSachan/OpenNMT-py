...
class TransformerEncoder(EncoderBase):
    ...
    def init_new(self, num_layers, hidden_size, dropout, embeddings):
        # super(TransformerEncoder, self).__init__()
    ...
    def forward_new(self, input, lengths=None, hidden=None):
        """ See :obj:`EncoderBase.forward()`"""
        self._check_args(input, lengths, hidden)
...
class TransformerDecoder(nn.Module):
    ...
    def init_new(self, num_layers, hidden_size, attn_type, copy_attn, dropout, embeddings):
        # super(TransformerDecoder, self).__init__()
        # Basic attributes.
        self.decoder_type = 'transformer'
        self.num_layers = num_layers
    ...
    def forward_new(self, input, context, state, context_lengths = None):
        # CHECKS
        assert isinstance(state, TransformerDecoderState)
        input_len, input_batch, _ = input.size()
        contxt_len, contxt_batch, _ = context.size()
        ...
        xy_mask = self.make_attention_mask(tgt_words, src_words)
        yy_mask = self.make_attention_mask(tgt_words, tgt_words)
        yy_mask *= self.make_history_mask(tgt_words)
